...
export default function RetoPage() {
  ...
  useEffect(() => {
    async function fetchReto() {
      ...
      if (submissions && submissions.length > 0) {
        ...
        const completo = ultima.resultado.every((r: string) => r.includes('✅'))
        if (completo) {
          setResuelto(true)
  ...
  const ejecutar = async () => {
    ...
    try {
      ...
      for (let i = 0; i < reto.test_cases.length; i++) {
        const { input, output } = reto.test_cases[i]
        const res = funcion(...input)
        const correcto = res === output
        resultados.push(
          `Caso ${i + 1}: ${correcto ? '✅' : `❌ Esperado ${output}, recibido ${res}`}`
        )
      }
    } catch (e) {
      resultados.push(`⚠️ Error al ejecutar el código: ${(e as Error).message}`)
    }
    ...
    if (error) console.error('Error al guardar envío:', error)
